...
const WalletRewards = ({ navigation }) => {
  ...
  const fetchTransactions = async (pageNumber = 1, isRefresh = false, isLoadMore = false) => {
    try {
      ...
      const response = await getRecords(
        { page: pageNumber - 1 }, // API might expect 0-based pages
        userToken, 
  ...
  const renderTransactionItem = ({ item: txn }) => {
    ...
    const detailsHeight = animValue.interpolate({
      inputRange: [0, 1],
      outputRange: [0, 160], // Approximate height of expanded details
    });
      ...
      {loading && transactions.length === 0 ? (
        ...
        <FlatList
          ...
          contentContainerStyle={[
            ...
            groupedTransactions.length === 0 && styles.emptyListContainer
          ]}
        />
      )}
    </View>
