...
export default function ComplaintScreen() {
  ...
  const fetchComplaints = async (pageNumber = 1, isRefresh = false) => {
    try {
      ...
      const response = await getRecords(
        ...
        userToken,
        'api/customer/complaint/getAllUserId'
      );
  ...
  const handleLoadMore = useCallback(() => {
    if (!isLoadingMore && hasMoreData && currentPage < totalPages) {
      ...
      console.log(`Loading more complaints: page ${nextPage}`);
  ...
  const formatDateTime = (date, time) => {
    if (date && time) {
      return `${dayjs(date).format('DD MMM YYYY')} at ${time.split('.')[0]}`;
    }
    return 'N/A';
  };
        ...
        {item.reply && (
          <View style={styles.replySection}>
            <View style={styles.replyHeader}>
              <Ionicons name="chatbubble-ellipses-outline" size={18} color="#000000" />
  ...
  const renderPaginationControls = () => {
    ...
    return (
      <View style={styles.paginationContainer}>
        ...
        >
          <Ionicons name="play-back" size={18} color={currentPage === 1 ? '#CCCCCC' : '#000000'} />
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          disabled={currentPage === totalPages}
        >
          <Ionicons name="play-forward" size={18} color={currentPage === totalPages ? '#CCCCCC' : '#000000'} />
        </TouchableOpacity>
      </View>
    );
  };
    ...
    return (
      <div style={{ 
        ...
        <div style={{ flexShrink: 0 }}>
          <CommonHeader2 heading="Complaints" goback="Home" />
        </div>
