...
export default function OtpValidateScreen({ route, navigation }) {
  ...
  const verifyLoginOTP = useCallback(async (otpCode, token) => {
    ...
    try {
      const apiResponse = await verifyOTP(otpCode, code, token);
      ...
      if (apiResponse?.status === 'success') {
        ...
        if (verified_status !== 1) {
          ...
          navigation.navigate('aadhaar_number', {
        ...
        // Complete login
        login(sessionToken, userData, permanentToken);
  ...
  }, [code, login, updateUserData, navigation]);
